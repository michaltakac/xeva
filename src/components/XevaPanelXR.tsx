...
export function XevaPanelXR({
  ...
  const { isGrabbed: controllerGrabbed, isHovered: controllerHovered } = useXRGrab(groupRef, {
    ...grabOptions,
    onGrab: () => {
      setIsHighlighted(true)
      onGrab?.()
  ...
  const panelBackgroundColor = useMemo(() => {
    ...
    const opacity = Math.round(backgroundOpacity * 255).toString(16).padStart(2, '0')
  ...
  const showHandIndicators = leftPinching || rightPinching || leftPointing || rightPointing
  const showDualHandIndicators = dualHandMode && (isLeftHandGrabbing || isRightHandInteracting)
  ...
  const renderControl = (control: typeof controls[0]) => {
    ...
    switch (control.type) {
      ...
      case 'boolean':
        return (
          <Container key={pathStr} flexDirection="row" alignItems="center" gap={16} minHeight={50}>
            <Toggle
              ...
              <Container flexDirection="row" gap={8}>
                ...
                {isGrabbed && (
                  <Container 
                    width={16} 
                    height={16} 
                    backgroundColor={dualHandMode && isLeftHandGrabbing ? "#a78bfa" : "#4ade80"} 
                    borderRadius={8}
                  />
                )}
