...
export function useHandTracking(
  ...
  const [grabbedByHand, setGrabbedByHand] = useState<'left' | 'right' | null>(null)
  
  const pinchPositions = useRef({ left: new THREE.Vector3(), right: new THREE.Vector3() })
  const pointDirections = useRef({ left: new THREE.Vector3(), right: new THREE.Vector3() })
  ...
  const calculatePointDirection = (hand: THREE.XRHandSpace): THREE.Vector3 => {
    ...
    const indexTip = (hand.joints as any)[HAND_JOINTS.INDEX_TIP]
    const indexProximal = (hand.joints as any)[HAND_JOINTS.INDEX_PHALANX_PROXIMAL]
  ...
  const detectGesture = (hand: THREE.XRHandSpace): HandGesture | null => {
    ...
    if (gestures.point) {
      ...
      if (indexTip && indexBase && middleTip && middleBase) {
        ...
        const indexExtended = tempVector.current.distanceTo(tempVector2.current)
        
        middleTip.getWorldPosition(tempVector.current)
        middleBase.getWorldPosition(tempVector2.current)
        const middleExtended = tempVector.current.distanceTo(tempVector2.current)
  ...
  const getPinchPosition = (hand: THREE.XRHandSpace): THREE.Vector3 => {
    ...
    if (thumbTip && indexTip) {
      thumbTip.getWorldPosition(tempVector.current)
      indexTip.getWorldPosition(tempVector2.current)
      position.addVectors(tempVector.current, tempVector2.current).multiplyScalar(0.5)
    }
