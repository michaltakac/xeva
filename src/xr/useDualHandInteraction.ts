...
export function useDualHandInteraction(
  ...
  >(null);
  const [hoveredControl, setHoveredControl] = useState<THREE.Object3D | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const handleGrabStart = (
      event: THREE.Event & {
        target: THREE.XRTargetRaySpace | THREE.XRGripSpace;
      },
    ) => {
      if (isGrabbed) return;
      ...
      // Check distance
      const controllerPos =
        grabButton === "squeeze" ? controller.grip : controller.controller;
      const distance = controllerPos.position.distanceTo(
  ...
  useEffect(() => {
    ...
    const handleInteractEnd = (
      _event: THREE.Event & { target: THREE.XRTargetRaySpace },
    ) => {
  ...
  const createInteractionRay = useCallback(() => {
    ...
    const points = [controller.controller.position, hoveredControl.position];
