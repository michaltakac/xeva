...
export function useXRGrab(
  ...
  useEffect(() => {
    ...
    const handleSelectStart = (event: THREE.Event & { target: THREE.XRTargetRaySpace }) => {
      if (isGrabbed) return
      ...
      if (intersects.length > 0) {
        const distance = controller.controller.position.distanceTo(ref.current.position)
    ...
    const handleSelectEnd = (event: THREE.Event & { target: THREE.XRTargetRaySpace }) => {
      if (!isGrabbed) return
    ...
    controllers.forEach((controller: any) => {
      controller.controller.addEventListener('selectstart', handleSelectStart as any)
      controller.controller.addEventListener('selectend', handleSelectEnd as any)
    })

    return () => {
      controllers.forEach((controller: any) => {
        controller.controller.removeEventListener('selectstart', handleSelectStart as any)
        controller.controller.removeEventListener('selectend', handleSelectEnd as any)
      })
    }
  }, [enabled, session, isGrabbed, grabbedBy, controllers, ref, applyHapticFeedback])

  // Handle squeeze events (grip press) for more natural grabbing
  useEffect(() => {
    if (!enabled || !session || !ref.current) return

    const handleSqueezeStart = (event: THREE.Event & { target: THREE.XRGripSpace }) => {
      if (isGrabbed) return
      ...
      const distance = controller.grip.position.distanceTo(ref.current.position)
      
      if (distance <= 0.3) { // Close proximity for grip grab
        // Calculate grab offset
        tempMatrix.current.copy(controller.grip.matrixWorld)
        tempMatrix.current.invert()
        tempMatrix.current.multiply(ref.current.matrixWorld)
    ...
    const handleSqueezeEnd = (event: THREE.Event & { target: THREE.XRGripSpace }) => {
      if (!isGrabbed) return
    ...
    controllers.forEach((controller: any) => {
      controller.grip.addEventListener('squeezestart', handleSqueezeStart as any)
      controller.grip.addEventListener('squeezeend', handleSqueezeEnd as any)
    })

    return () => {
      controllers.forEach((controller: any) => {
        controller.grip.removeEventListener('squeezestart', handleSqueezeStart as any)
        controller.grip.removeEventListener('squeezeend', handleSqueezeEnd as any)
      })
    }
  }, [enabled, session, isGrabbed, grabbedBy, controllers, ref, applyHapticFeedback])
