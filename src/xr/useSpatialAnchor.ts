...
export function useSpatialAnchor(
  ...
  const { enabled = true, anchor, onAnchorUpdate, debugMode = false } = options;
  ...
  useEffect(() => {
    if (!enabled || !session || !referenceSpace || anchor?.type === "fixed")
      return;
  ...
  const snapToNearestSurface = useCallback(() => {
    ...
    directions.forEach((direction) => {
      ...
      if (intersects.length > 0 && intersects[0].distance < nearestDistance) {
        ...
        nearestNormal =
          intersects[0].face?.normal.clone() || direction.clone().negate();
      }
    });
