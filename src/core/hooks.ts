...
export function useXRControls<T extends ControlsSchema>(
  ...
  const folderName =
    typeof schemaOrFolder === "string" ? schemaOrFolder : undefined;
  const schema = (
    typeof schemaOrFolder === "string" ? maybeSchema : schemaOrFolder
  ) as T;
  ...
  useEffect(() => {
    if (!registeredRef.current) {
      ...
      store
        .getState()
        .registerSchema(schemaId.current, finalSchema as ControlsSchema, {
          order: options.order,
        });
  ...
  const values = useMemo(() => {
    ...
    function buildProxy(obj: ControlsSchema, path: string[] = []): any {
      ...
      Object.entries(obj).forEach(([key, value]) => {
        const currentPath = [
          ...path,
          path.length === 0 ? schemaId.current : key,
        ];
    ...
    return buildProxy(
      folderName ? { [folderName]: { folder: schema } } : schema,
    ) as ExtractValues<T>;
