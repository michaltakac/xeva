...
interface XRControlsActions {
  registerSchema: (id: string, schema: ControlsSchema, options?: { order?: number }) => void
...
function parseSchema(
  ...
  Object.entries(schema).forEach(([key, value]) => {
    ...
    if (control) {
      ...
      if (control.type === 'folder' && typeof value === 'object' && 'folder' in value) {
...
function detectType(value: any, config: Partial<ControlConfig>): ControlType {
  ...
  if (typeof value === 'string' && value.match(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/)) {
    return 'color'
  }
