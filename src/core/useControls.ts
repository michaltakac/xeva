...
// Types
export type ControlValue = string | number | boolean | { x: number; y: number; z: number } | (() => void)
...
interface ControlData {
  ...
  value: ControlValue
  type: 'number' | 'boolean' | 'string' | 'select' | 'color' | 'vector3' | 'button' | 'folder'
}
...
function inferControlType(value: any, config: Partial<ControlConfig> = {}): ControlData['type'] {
  // Button
  ...
  // Vector3
  if (typeof value === 'object' && value !== null && 'x' in value) return 'vector3'
...
function parseSchema(schema: ControlsSchema, parentPath: string[] = []): {
  controls: Map<string, ControlData>
  values: Record<string, ControlValue>
  ...
  Object.entries(schema).forEach(([key, item]) => {
    ...
    if (isFolder) {
      ...
      folderResult.controls.forEach((control, key) => controls.set(key, control))
      Object.assign(values, folderResult.values)
      folderResult.folders.forEach((children, key) => folders.set(key, children))
...
// Global store
export const useXevaStore = create<XevaStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    getFolder: (path) => {
      ...
      return children.map(childPath => get().controls.get(childPath)!).filter(Boolean)
    },
...
export function useControls(nameOrSchema: string | ControlsSchema, schema?: ControlsSchema) {
  const name = typeof nameOrSchema === 'string' ? nameOrSchema : 'controls'
  const actualSchema = typeof nameOrSchema === 'string' ? schema! : nameOrSchema
  
  const store = useXevaStore()
    ...
    return () => {
      ...
      if (initialized.current) {
        ...
        useXevaStore.setState(state => {
          ...
          return { controls: newControls, values: newValues, folders: newFolders }
        })
  ...
  const values = useXevaStore(state => state.getValues(name))
  ...
  return useMemo(() => {
    ...
    Object.keys(values).forEach(key => {
      ...
      if (setters[key]) {
        Object.defineProperty(result, `set${key.charAt(0).toUpperCase()}${key.slice(1)}`, {
          value: setters[key],
          enumerable: false
        })
      }
